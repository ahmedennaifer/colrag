...
async def get_wikipedia_article(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(detail=f"Failed to create workspace for article {wiki.title}", status_code=500)
        ...
        raise HTTPException(
            status_code=404,
            detail=f"Wikipedia article not found: {wiki.title}"
        )
    except Exception as e:
        raise HTTPException(
            status_code=500,
...
async def ask_wikipedia(msg: Message) -> Dict[str, Any]:
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error querying Wikipedia content: {str(e)}"
        )
